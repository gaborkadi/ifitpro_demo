...
public class CatmullRom {
	...
	public CatmullRom(Vector3[] controlPoints, bool closedLoop) {
	...
	public CatmullRom(Segment[] controlPoints, bool closedLoop) {
		if (controlPoints == null || controlPoints.Length <= 2 || resolution < 2) {
			throw new ArgumentException("Catmull Rom Error: Too few control points or resolution too small");
		}
		...
		this.controlPoints = new Vector3[controlPoints.Length];
		for (int i = 0; i < controlPoints.Length; i++) {
			this.controlPoints[i] = controlPoints[i].Direction;
		}
	...
	public CatmullRomPoint GetPoint(float t) {
		t %= 1f;
		int index = Mathf.Clamp(Mathf.RoundToInt((splinePoints.Length - 1) * t), 0, splinePoints.Length - 1);
		return splinePoints[index];

		/*float indexArea = splinePoints.Length - 1) * t;
		int lowerPointIndex = Mathf.Clamp(Mathf.FloorToInt(indexArea), 0, splinePoints.Length - 1);
		int higherPointIndex = Mathf.Clamp(Mathf.CeilToInt(indexArea), 0, splinePoints.Length - 1);

		if (lowerPointIndex == higherPointIndex) {
			return splinePoints[lowerPointIndex];
		}
		else {
			return new CatmullRomPoint(
				(splinePoints[lowerPointIndex].position + splinePoints[higherPointIndex].position) / 2f,
				(splinePoints[lowerPointIndex].tangent + splinePoints[higherPointIndex].tangent) / 2f,
				(splinePoints[lowerPointIndex].normal + splinePoints[higherPointIndex].normal) / 2f
			);
		}*/
	}
